...
// Processar ações em massa (ativar/desativar/excluir)
    ...
    if (empty($ids)) {
        ...
        try {
            if ($bulkAction === 'ativar' || $bulkAction === 'desativar') {
                ...
                $bloqueados = 0;
                foreach ($ids as $id) {
                    // Verificar associação com grupos
...
try {
    ...
    // Filtrar por associação a grupos usando HAVING sobre o agregado total_grupos
    ...
    // Contagem total para paginação (subconsulta sobre a mesma query)
