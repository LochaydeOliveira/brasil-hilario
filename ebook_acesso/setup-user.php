...
// Mesmo comportamento do registrar.php (primeiro usuário sem secret)
$temUsuarios = 0;
try { $temUsuarios = (int)$pdo->query('SELECT COUNT(*) FROM usuarios')->fetchColumn(); } catch (Throwable $e) { $temUsuarios = 0; }
if ($temUsuarios > 0 && (($_GET['secret'] ?? '') !== ADMIN_SECRET)) { header('Location: index.php'); exit; }
...
<head>
    ...
    <title>Configurar Usuário Inicial</title>
